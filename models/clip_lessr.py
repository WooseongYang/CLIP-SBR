...
class CLIP_LESSR(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.community_prompt = nn.Embedding(self.n_communities + 1, embedding_dim, padding_idx=0)

        # gate layers
        self.gate_layer_item = nn.Linear(embedding_dim, embedding_dim)
        self.gate_layer_prompt = nn.Linear(embedding_dim, embedding_dim)
    ...
    def forward(self, x, edge_index_EOP, edge_index_shortcut, batch, is_last):
        ...
        ###
        x_community = torch.tensor([self.partition.get(item.item(), self.most_frequent_partition) for item in x], device='cuda:0')
        
        cprompt = self.community_prompt(x_community)
        
        # Compute gating weights
        gate_weight_item = self.sigmoid(self.gate_layer_item(feat))
