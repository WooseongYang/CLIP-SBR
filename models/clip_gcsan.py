...
class CLIP_GCSAN(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        super(CLIP_GCSAN, self).__init__(config, dataset)
        ...
        self.community_prompt = nn.Embedding(self.n_communities + 1, self.hidden_size, padding_idx=0)

        # gate layers
        self.gate_layer_item = nn.Linear(self.hidden_size, self.hidden_size)
        self.gate_layer_prompt = nn.Linear(self.hidden_size, self.hidden_size)
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        # gating
        gated_input = gate_weight_item * hidden + gate_weight_cprompt * cprompt

        hidden = self.gnn(A, gated_input)
